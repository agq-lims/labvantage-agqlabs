 ...
 */
public class presupuestomodificacionanterior extends BaseAction {
    ...
    public void processAction(PropertyList props) throws SapphireException {
        ...
        if (!this.error) {
            ...
            plData.setProperty("estado", "Aceptado");
            plData.setProperty("fecha_aceptacion", props.getProperty("FechaMaxValidez", "").replace("T", " "));
            plData.setProperty("cliente_facturar", obtenerAddressIdCliente(props.getProperty("CodClienteFactura", "")));
...
//                    }
                ...
                java.util.logging.Logger.getLogger(presupuestomodificacionanterior.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        //
        if (!this.error) {
